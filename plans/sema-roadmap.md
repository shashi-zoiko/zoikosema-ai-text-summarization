...
## Remaining-gap sweep (2026-07-17) — the other 6 of 7 triaged buildable items

Same triage that picked Work Graph governance edges (above) also flagged 6 more genuinely-buildable gaps. User said "complete everything" — all 6 built, each its own branch/commit, merged in sequence:

- **"Managed by Zoiko Group" policy chip + agent-touched violet signal** (`sema/policy-chip-agent-violet-signal`, merged): `Badge` gained an `agent` tone using a new `--c-agent-violet` token pair (spec's exact `#5A45D6` hex in light theme), applied to `ReviewQueue.jsx`'s Agent-proposed badge plus a violet card border so agent involvement reads without opening the item (spec's identify-without-opening requirement). `Governance.jsx`'s per-category panel shows "Managed by Zoiko Group" once a real `PolicyVersion` exists for that category. No other client surface renders `proposed_by_agent`/agent-drafted content yet, so the violet signal is scoped to where a real consumer exists (ReviewQueue) — same "don't build for a caller that doesn't exist" discipline as everywhere else in this build.
- **DLP keyword config + delayed-send buffer admin settings** (`sema/dlp-keyword-buffer-settings`, merged): both were hardcoded module constants shared by every tenant. New append-only `connect_mail_governance_settings` table (versioned like `PolicyVersion`) holds a tenant's sensitive-keyword list + buffer min/max/default bounds; `dlp.scan()` now takes an optional keyword override, `mail_service.send.py`'s `stage_send` resolves the tenant's configured bounds instead of fixed constants. New "Mail settings" panel in `Governance.jsx`. Verified end-to-end: defaults-when-unset, versioning, DLP scanning against real tenant keywords vs. module defaults, append-only enforcement, bounds validation.
- **Structured reasoning trace** (`sema/structured-reasoning-trace`, merged): `reasoning_trace_ref` was a bare `"func_name:model"` string with none of spec §5.1's required structure. New append-only `connect_reasoning_traces` table holds real rationale/source-nodes/tool-chain/model/confidence/uncertainty-markers, wired into all 3 real agent-generation staging call sites with genuine (not fabricated) content — no invented confidence score, since `ai.py`'s prompts don't currently elicit one (disclosed gap). `get_reasoning_trace()` redacts rationale/source/tool-chain for a plain `member` role per spec §5.4's "redactable per reviewer access"; owner/admin/personal see everything. `ReviewQueue.jsx` renders the structured trace instead of the raw ref string. Export tooling for Enterprise audit roles explicitly not built — no Enterprise tier exists in this codebase yet.
- **Task rollback / restore previous version** (`sema/task-rollback-restore-version`, merged): spec §5.2's Task row was explicitly deferred in Phase 2 slice 8. New append-only `connect_task_versions` side-snapshot table keyed by `Task`'s own stable id — deliberately NOT a version-chain conversion of `Task` itself (Work Graph's task node resolver and every `derived_from`/`mutated` edge key off a stable `Task.id`; multiplying rows per edit the way `NativeCalendarEvent` does would break that identity everywhere it's already referenced). `restore_previous_task_version()` re-applies the prior snapshot onto the live row and records that restore as a further new version. A newly-created task (only its v1 snapshot) correctly refuses to restore, directing the caller to dismiss instead — matches spec's own "restore OR delete if newly created" framing. `ai_workflows.py`'s follow-up-tasks staging now uses `rollback_descriptor="restore_previous_version"` instead of `"no_rollback"`. No client UI added — no client caller exists for any Calendar Service task endpoint on this branch yet (same gap the Client-UI audit above already flagged for the wider calendar REST surface).
- Every slice in this sweep: its own branch cut from `feature/sema-calendar-mail`, verified against real Postgres via a dedicated scratch script (not mocked), then merged back before the next slice started — same one-slice-one-branch cadence as the rest of this build. Client changes additionally passed `npm run lint`/`npm run build` clean.
